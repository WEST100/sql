...
set commission =  
			case
                ...
                when salary > 6000 and salary < 9000 then 15
                when salary >= 9000 then 20
                end;
                
-- Создать поле finalsalary и заполнить формулой salary + salary * commission / 10. 
-- Проверить commission на null, так чтобы в случае неопределенности finalprice
-- принимал значение salary. 

alter table employees
add finalsalary int;

update employees
set finalsalary = salary + salary * commission / 10;

update employees
set finalsalary =  
			case
				when finalsalary is null then salary
                end;
...
-- Заполнить поле chkemail. Если длина email меньше 3, значением chkemail будет
--  'Введите больше символов', если длина email больше/равно 3 и меньше 7,  
-- значением chkemail будет 'Кол/во символов совпадает', если длина email больше/равно 7, 
--  значением chkemail будет 'Введите меньше символов'.

update employees
set chk_email =  
			case
				when length(email) < 9 then 'Введите больше символов'
                when length(email) >= 3 and length(email) < 7 then 'Колво символов совпадает'
